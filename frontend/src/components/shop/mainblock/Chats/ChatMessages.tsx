  ...
  return (
    ...
    <div className="space-y-4 max-w-3xl mx-auto">
      {messages.map((message) => (
        <div
          key={message._id}
          className={`flex ${message.senderId == shopId? "justify-end":""}`}
        >
          <div
            className={`max-w-[85%] sm:max-w-[70%] shadow-md rounded-t-lg rounded-s-lg px-4 py-1 ${message.senderId === shopId? "bg-primary text-primary-foreground": "bg-muted"}`}
          >
            {message.image && (
