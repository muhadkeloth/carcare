...
const OrderDetails: React.FC<BookingDetailsProps> = ({
  bookingDetails,
  handlesetPickupData,
}) => {
  ...
  const togglePickupStatus = async (pickupId: string, status: string) => {
    try {
      const response = await togglepickupStatus(pickupId, status);
