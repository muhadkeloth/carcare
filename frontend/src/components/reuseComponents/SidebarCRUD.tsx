...
import {  faArrowRightFromBracket, faTimes, } from '@fortawesome/free-solid-svg-icons';
...
const SidebarCRUD:React.FC<SidebarProps & CRUDbarProps> = ({ title, sections, activeSection, handleActiveSection, handleLogout, showMenu }) => {
  return (
    <>
      <div className="flex w-full mb-6 pe-3 items-center justify-between z-11">
        <h2 className="text-xl font-bold ">{title}</h2>
        {showMenu && ( <FontAwesomeIcon icon={faTimes} onClick={() => handleActiveSection(activeSection)} className='hover:cursor-pointer' /> )}
      </div>
