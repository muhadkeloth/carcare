...
const MoreInfo: React.FC = () => {
  ...
  const [editedShopUser, setEditedShopUser] = useState<{title: string;discript: string;about: string;}>({
    title: shopUserDetails?.discription?.title || "",
    discript: shopUserDetails?.discription?.discript || "",
    about: shopUserDetails?.about || "",
  });
  ...
  const handleSaveChanges = async () => {
    ...
    setEditedShopUser((prev) => ({
      ...
      discript: prev.discript.trim(),
      about: prev.about.trim(),
    }));
    setErrors(null);
    if (!editedShopUser) {
    ...
    if (!editedShopUser?.discript || editedShopUser?.discript.length < 10) {
      setErrors((prev) => ({
        ...prev,
        discript: "Discript is required and least 10 characters.",
      }));
      flag = true;
    }
    if (!editedShopUser?.about || editedShopUser?.about.length < 10) {
      setErrors((prev) => ({
        ...prev,
        about: "About is required and least 10 characters.",
      }));
      flag = true;
    }
    if (flag) return;
      ...
      <div className="flex flex-col flex-grow sm:ms-4">
        <form className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className={`w-full border ${
                  errors?.title ? "border-gray-500" : "border-gray-300"
                } rounded-md p-2`}
          ...
          <div>
            ...
            <textarea
              className={`w-full border ${
                errors?.discript ? "border-gray-500" : "border-gray-300"
              } rounded-md p-2`}
            ...
            ></textarea>
            {errors?.discript && (
              <span className="text-red-500 text-sm">{errors?.discript}</span>
            )}
          </div>
          ...
          <div>
            ...
            <textarea
              className={`w-full border ${
                errors?.about ? "border-gray-500" : "border-gray-300"
              } rounded-md p-2`}
